...
public class LcmTestClient implements LCMSubscriber {
  ...
  private static <T> void checkField(T actual, T expected, String field) {
    if (!actual.equals(expected)) {
      throw new IllegalArgumentException(
          String.format(
              "%s : Expected %s, got %s", field, String.valueOf(expected), String.valueOf(actual)));
    }
  }
  ...
  private class MessageTester<MsgType extends lcm.lcm.LCMEncodable> implements LCMSubscriber {
    ...
    MessageTester(
        LCM lcm,
        String name,
        MessageMakerChecker<MsgType> makerChecker,
        int numIterations,
        Class<MsgType> c) {
      lcm_ = lcm;
        ...
        for (int a_iter = 0; a_iter < reply.size_a; a_iter++) {
          for (int b_iter = 0; b_iter < reply.size_b; b_iter++) {
            for (int c_iter = 0; c_iter < reply.size_c; c_iter++) {
              checkField(
                  reply.data[a_iter][b_iter][c_iter],
                  n,
  ...
  private class CrossPackageMakerChecker implements MessageMakerChecker<cross_package_t> {
    private PrimitivesMakerChecker primitivesMakerChecker_ = new PrimitivesMakerChecker();
  ...
  public int runTests() {
    try {
      ...
      MessageTester<primitives_t> primitives_tester =
          new MessageTester<primitives_t>(
              lcm_, "lcmtest_primitives_t", new PrimitivesMakerChecker(), 1000, primitives_t.class);
      if (!primitives_tester.run()) {
        return 1;
      }

      MessageTester<primitives_list_t> primitives_list_tester =
          new MessageTester<primitives_list_t>(
              lcm_,
              "lcmtest_primitives_list_t",
              new PrimitivesListMakerChecker(),
              100,
              primitives_list_t.class);
      if (!primitives_list_tester.run()) {
        return 1;
      }

      MessageTester<node_t> node_tester =
          new MessageTester<node_t>(
              lcm_, "lcmtest_node_t", new NodeMakerChecker(), 7, node_t.class);
      if (!node_tester.run()) {
        return 1;
      }

      MessageTester<multidim_array_t> multidim_array_tester =
          new MessageTester<multidim_array_t>(
              lcm_,
              "lcmtest_multidim_array_t",
              new MultidimArrayMakerChecker(),
              5,
              multidim_array_t.class);
      if (!multidim_array_tester.run()) {
        return 1;
      }

      MessageTester<cross_package_t> cross_package_tester =
          new MessageTester<cross_package_t>(
              lcm_,
              "lcmtest2_cross_package_t",
              new CrossPackageMakerChecker(),
              100,
              cross_package_t.class);
