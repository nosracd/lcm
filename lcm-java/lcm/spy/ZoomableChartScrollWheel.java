...
public class ZoomableChartScrollWheel extends ZoomableChart {
  ...
  /**
   * Creates a new frame for this trace. Called either by ObjectPanel to create a new graph or from
   * the right-click menu to move a trace to a new graph.
   *
  ...
  /**
   * Gets the next color for a new trace. Use this to keep colors as different as possible.
  ...
  /**
   * Updates the right click menu to allow for moving traces around. Should be called immediately
   * after adding a new trace.
   */
  public void updateRightClickMenu() {
    ...
    while (iter.hasNext()) {
        ...
        newItem.addActionListener(
            new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                ...
                ZoomableChartScrollWheel.this.addTrace(
                    trace, ZoomableChartScrollWheel.this.getAxisX(), newAxis);
                ZoomableChartScrollWheel.this.updateRightClickMenu();
              }
            });
      ...
      moveWindowItem.addActionListener(
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
  ...
  /**
   * Saves the time this window was last in focus. Allows us to put new traces on the last chart
   * that was in focus
   *
  ...
  public void addFrameFocusTimer(JFrame frame) {
    ...
    this.frame.addWindowFocusListener(
  ...
  private void dragChart(MouseEvent e) {
    ...
    if (Double.isNaN(mouseDownMinX)
        || Double.isNaN(mouseDownMaxX)
        || Double.isNaN(mouseDownMinY.get(0))
        || Double.isNaN(mouseDownMaxY.get(0))
        || Double.isNaN(deltaX)
        || Double.isNaN(deltaY)
        || Double.isInfinite(mouseDownMinX)
        || Double.isInfinite(mouseDownMaxX)
        || Double.isInfinite(mouseDownMinY.get(0))
        || Double.isInfinite(mouseDownMaxY.get(0))
        || Double.isInfinite(deltaX)
        || Double.isInfinite(deltaY)) {
      return;
    }

    zoom(
        mouseDownMinX - deltaX,
        mouseDownMaxX - deltaX,
        mouseDownMinY.get(0) + deltaY,
        mouseDownMaxY.get(0) + deltaY);
    ...
    for (int i = 0; i < rightYAxis.size(); i++) {
      ...
      zoom(
          axis,
          axis.translateValueToPx(mouseDownMinY.get(i + 1) + deltaYRight),
          axis.translateValueToPx(mouseDownMaxY.get(i + 1) + deltaYRight));
    }
  }

  /**
   * Variable-width x-axis formatting causes jumps when data is "rolling in" because the labels
   * change width, which causes the X-axis to change width which causes visual disruption. When
   * using zoomAll, change to a fixed width format on the x-axis.
   */
  ...
  /** When data is not "rolling in" use a varible format for maximum flexibility in display. */
  ...
  public class MyMouseWheelListener implements MouseWheelListener {
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
      ...
      double xPercent =
          ((double) e.getX() - (double) chart.getXChartStart())
              / (double) (chart.getXChartEnd() - chart.getXChartStart());
      double yPercent =
          ((double) e.getY() - (double) chart.getYChartEnd())
              / (double) (chart.getYChartStart() - chart.getYChartEnd());
