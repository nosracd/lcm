...
/**
 * Panel that displays general data for lcm types. Viewed by double-clicking or right-clicking and
 * selecting Structure Viewer on the channel list.
 */
public class ObjectPanel extends JPanel {
  ...
  /**
   * If given a viewport, the object panel can make smart decisions to not draw graphs that are
   * currently outside of the user's view
   *
  ...
  /**
   * Opens a detailed, interactive chart for a data stream. If the data is already displayed in a
   * chart, brings that chart to the front instead.
   *
   * @param data data channel to display
   * @param openNewChart set to true to force opening of a new chart window, false to add to an
   *     already-open chart (if one exists)
  ...
  public void displayDetailedChart(SparklineData data, boolean openNewChart, boolean newAxis) {
    ...
    if (data.chart == null) {
      ...
      System.out.println(
          "Warning: detailed chart display requested on uninitialized chart " + data.name);
      return;
    }
  ...
  class PaintState {
    Color indentColors[] =
        new Color[] {new Color(255, 255, 255), new Color(230, 230, 255), new Color(200, 200, 255)};
    Graphics g;
    FontMetrics fm;
    JPanel panel;
    ...
    public void drawStrings(String type, String name, String value, boolean isstatic) {
      ...
      Font of = g.getFont();
      if (isstatic) g.setFont(of.deriveFont(Font.ITALIC));
     ...
     * @param sec index of section this row is in, used to determine if this row should be
     *     highlighted because it is under the mouse cursor.
     */
    public void drawStringsAndGraph(Class cls, String name, Object o, boolean isstatic, int sec) {
      ...
      if (currentlyHoveringSection != null
          && cs == currentlyHoveringSection
      ...
      if (cls.equals(Byte.TYPE)) {
        g.drawString(
            String.format(
                "0x%02X   %03d   %+04d   %c",
                (o), ((Byte) o).intValue() & 0x00FF, (o), ((Byte) o) & 0xff),
            x[2],
            y);
      } else {
    ...
    public void DrawSparkline(int x, int y, ITrace2D trace, boolean isHovering) {
      ...
      if (trace.getMaxY() == trace.getMinY()) {
        ...
        g2.drawLine(
            leftLineX,
            y - (int) ((double) height / (double) 2),
            x + (int) width,
            y - (int) ((double) height / (double) 2));
        g2.setColor(pointColor);
        g2.fillOval(
            x + (int) width - 1,
            y - (int) ((double) height / (double) 2) - 1,
            circleSize,
            circleSize);
        return;
      }
    ...
    public void beginColorBlock() {
      ...
      g.fillRect(
          x[0] + indent_level * indentpx - indentpx / 2,
          y - fm.getMaxAscent(),
          getWidth(),
          getHeight());
      g.setColor(Color.black);
  ...
  /**
   * Updates visibleSparklines to reflect the data that is near the user's view at the current time.
   *
  ...
  public void paint(Graphics g) {
    ...
    if (o != null) paintRecurse(g, ps, "", o.getClass(), o, false, -1);
  ...
  void paintRecurse(
      Graphics g, PaintState ps, String name, Class cls, Object o, boolean isstatic, int section) {
    if (o == null) {
    ...
    if (cls.isPrimitive() || cls.equals(Byte.TYPE)) {
      ...
      if (data == null) {
        // we may or may not draw this depending on if it is near the view but we need to keep track
        // of it
      ...
      for (int i = 0; i < sz; i++)
        paintRecurse(
            g, ps, name + "[" + i + "]", cls.getComponentType(), Array.get(o, i), isstatic, sec);
      ...
      for (Field f : fs) {
        try {
          paintRecurse(
              ...
              f.getType(),
              f.get(o),
              isstatic || ((f.getModifiers() & Modifier.STATIC) != 0),
              sec);
        } catch (Exception ex) {
  ...
  /**
   * Usually the paintRecurse method deals with searching through the incoming data but when the
   * window is minimized, we do not paint, causing the child graphs to stop updating. This method
   * performs that search and adds data to the graphs without actually drawing anything
   */
   ...
   * @param o
   */
  void UpdateGraphDataWithoutPaintRecurse(
      SparklineData sparklineToUpdate, String name, Class cls, Object o) {
    ...
    if (cls.isPrimitive() || cls.equals(Byte.TYPE)) {
      ...
      for (int i = 0; i < sz; i++)
        UpdateGraphDataWithoutPaintRecurse(
            sparklineToUpdate, name + "[" + i + "]", cls.getComponentType(), Array.get(o, i));
  ...
  /**
   * Initialize a chart. This should happen before you want to use the chart, either for storing
   * data or displaying data. It's best to delay the inits until you need them, because making a lot
   * of charts seems to slow down chart interactions.
   *
  ...
  class MyMouseAdapter extends MouseAdapter {
    /**
     * Handle mouse clicks. Either opens graphs if the user clicked on a row or toggles sections.
     *
  ...
  class MyViewportChangeListener implements ChangeListener {
    /**
     * Here we build a list of the items that are visible or are close to visible to the user. That
     * way, we can only update sparkline charts that are close to what the user is looking at,
     * reducing CPU load with huge messages
     *
