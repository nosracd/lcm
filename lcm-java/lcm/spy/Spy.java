        ...
        new MouseAdapter() {
          public void mouseClicked(MouseEvent e) {
            ...
            if (e.getButton() == 3) {
              ...
              if (!got_one) createViewer(channelList.get(row));
            }
          }
        });

    jif.addWindowListener(
        new WindowAdapter() {
    ...
    public Object getValueAt(int row, int col) {
      ...
      switch (col) {
        case 0:
          return cd.name;
        case 1:
          if (cd.cls == null) return String.format("?? %016x", cd.fingerprint);
  ...
  class MySubscriber implements LCMSubscriber {
    public void messageReceived(LCM lcm, String channel, LCMDataInputStream dins) {
      ...
      try {
        ...
        if (cd.viewer != null) cd.viewer.setObject(o, cd.last_utime);
  ...
  class HzThread extends Thread {
    ...
    public void run() {
      while (true) {
        ...
        channelTableModel.fireTableDataChanged();
        if (selrow >= 0) channelTable.setRowSelectionInterval(selrow, selrow);
