 ...
 *
 * <p>{@link LCM} normally delivers messages asynchronously by invoking the {@link
 * LCMSubscriber#messageReceived messageReceived} method on a subscriber as soon as a message is
 * received. This class provides an alternate way to receive messages by storing them in an internal
 * queue, and then delivering them to synchronously to the user.
 *
 * <p>The aggregator has configurable limits. If too many messages are aggregated without having
 * been retrieved, then older messages are discarded.
 */
public class MessageAggregator implements LCMSubscriber {
   ...
   *
   * <p>This is an alternative way to limit the messages stored by the aggregator. Messages are
   * discarded oldest-first to ensure that the total size of unretrieved messages stays under this
   * limit.
   *
   ...
   *
   * <p>Messages are discarded oldest-first to ensure that the number of unretrieved messages stays
   * under this limit.
   */
  public synchronized void setMaxMessages(int val) {
    max_queue_length = val;
  }
   ...
   *
   * @param timeout_ms Max # of milliseconds to wait for a message. If 0, then don't wait. If less
   *     than 0, then wait indefinitely.
