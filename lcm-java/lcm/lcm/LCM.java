...
public class LCM {
  /**
   * @return The value of the LCM_DEFAULT_URL environment variable if set or
   *     "udpm://239.255.76.67:7667".
   */
  public static String getDefaultURL() {
    String env = System.getenv("LCM_DEFAULT_URL");
    if (null == env || env.isEmpty()) return "udpm://239.255.76.67:7667";
    return env;
  }
  ...
  HashMap<String, ArrayList<SubscriptionRecord>> subscriptionsMap =
      new HashMap<String, ArrayList<SubscriptionRecord>>();
  ...
  /**
   * Create a new LCM object, connecting to one or more URLs. If no URL is specified, the
   * environment variable LCM_DEFAULT_URL is used. If that environment variable is not defined, then
  ...
  public LCM(String... urls) throws IOException {
    ...
    for (String url : urls) {
      ...
      if (protocol.equals("udpm")) providers.add(new UDPMulticastProvider(this, up));
      else if (protocol.equals("tcpq")) providers.add(new TCPProvider(this, up));
      else if (protocol.equals("file")) providers.add(new LogFileProvider(this, up));
      else if (protocol.equals("memq")) providers.add(new MemqProvider(this, up));
      else System.err.println("LCM: Unknown URL protocol: " + protocol);
    }
  }

  /**
   * Retrieve a default instance of LCM using either the environment variable LCM_DEFAULT_URL or the
  ...
  /**
   * Publish a string on a channel. This method does not use the LCM type definitions and thus is
   * not type safe. This method is primarily provided for testing purposes and may be removed in the
   * future.
   */
  ...
  /**
   * Publish an LCM-defined type on a channel. If more than one URL was specified, the message will
   * be sent on each.
   */
  ...
  /**
   * Publish raw data on a channel, bypassing the LCM type specification. If more than one URL was
   * specified when the LCM object was created, the message will be sent on each.
   */
  public synchronized void publish(String channel, byte[] data, int offset, int length)
      throws IOException {
    if (this.closed) throw new IllegalStateException();
    for (Provider p : providers) p.publish(channel, data, offset, length);
  }
   ...
   * function, the LCM instance should consume no resources, and cannot be used to receive or
   * transmit messages.
   */
