...
public class URLParser {
  ...
  public URLParser(String url) {
    ...
    if (network_args.length > 1) {
      ...
      for (int i = 0; i < keyvalues.length; i++) {
        ...
        if (toks.length != 2) System.err.println("Invalid key-value pair in URL : " + keyvalues[i]);
        else params.put(toks[0], toks[1]);
      }
    }
  }
  ...
  public String get(String key, String def) {
    if (params.get(key) == null) return def;
  ...
  public static void main(String args[]) {
    ...
    if (args.length < 1) {
      ...
      if (null != env) u = new URLParser(env);
      else {
