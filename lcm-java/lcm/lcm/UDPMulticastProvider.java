...
/**
 * LCM provider for the udpm: URL. All messages are broadcast over a pre-arranged UDP multicast
 * address. Subscription operations are a no-op, since all messages are always broadcast.
 *
 * <p>This mechanism is very simple, low-latency, and efficient due to not having to transmit
 * messages more than once when there are multiple subscribers. Since it uses UDP, it is lossy.
 */
public class UDPMulticastProvider implements Provider {
  ...
  public UDPMulticastProvider(LCM lcm, URLParser up) throws IOException {
    ...
    if (ttl == 0) System.err.println("LCM: TTL set to zero, traffic will not leave localhost.");
    else if (ttl > 1) System.err.println("LCM: TTL set to > 1... That's almost never correct!");
    else System.err.println("LCM: TTL set to 1.");
  ...
  class ReaderThread extends Thread {
    ...
    void handlePacket(DatagramPacket packet) throws IOException {
      LCMDataInputStream ins =
          new LCMDataInputStream(packet.getData(), packet.getOffset(), packet.getLength());
