...
public class LogFileProvider implements Provider {
  ...
  class ReaderThread extends Thread {
    ...
    void runEx() throws IOException, InterruptedException {
      ...
      while (lcm.getNumSubscriptions() == 0) Thread.sleep(10);
      ...
      while (true) {
        ...
        if (lastEventUtime > 0) {
          ...
          if (dt > 0 && speed > 0) delayAccumulator += dt / speed;
        }
        ...
        if (verboseAccumulator > 1.0 && verbose) {
          ...
          System.err.printf(
              "LogFile: rate = %8.3f, position = %8.3f %%\n",
              eventDt / verboseAccumulator, log.getPositionFraction() * 100.0);
