...
/**
 * LCM provider for the tcpq: URL. All messages are sent to a central "hub" process (that must be
 * started separately), which will relay the messages to all other processes. TCPService is an
...
public class TCPProvider implements Provider {
  ...
  public TCPProvider(LCM lcm, URLParser up) throws IOException {
    ...
    if (addrport.length == 1) {
      ...
      System.err.println(
          "TCPProvider: Don't know how to parse " + up.get("network", DEFAULT_NETWORK));
      System.exit(-1);
    }
  ...
  /**
   * Publish a message synchronously. However, if the server is not available, it will return
   * immediately.
   */
      ...
      while (!exit) {
        ...
        synchronized (this) {
          ...
          try {
            ...
            System.err.println(
                "lcm.TCPProvider: Unable to connect to " + inetAddr + ":" + inetPort);
            safeSleep(500);
