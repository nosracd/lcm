...
public class ParameterGUI {
  ...
  /**
   * This makes a text field yellow until they've pressed return; it's a reminder that you need to
   * hit return for the changes to your text field to apply.
   */
  ...
  class IntegerValue extends PValue {
    ...
    JLabel getLabel() {
      if (label == null) label = new JLabel("" + value);
      return label;
    }
    ...
    void setIntegerValue(int v) {
      ...
      value = v;
      if (slider != null) slider.setValue(v);
      if (textField != null) textField.setText("" + v);
      if (label != null) label.setText("" + v);
    ...
    void setEnabled(boolean v) {
      if (slider != null) slider.setEnabled(v);
      if (textField != null) textField.setEnabled(v);
    }
  }
  ...
  class DoubleValue extends PValue {
    ...
    void updateSliderValue() {
      ...
      if (slider != null) slider.setValue(ivalue);
      ...
      if (slider == null) {
        ...
        slider.addChangeListener(
            new ChangeListener() {
              public void stateChanged(ChangeEvent e) {
                setDoubleValue(
                    ((double) slider.getValue() + .5) / SLIDER_CLICKS * (max - min) + min);
              }
            });
      }
      return slider;
  ...
  class StringValue extends PValue {
    ...
    void updateIndex() {
      if (values != null) {
        ...
        for (int i = 0; i < values.length; i++) {
          if (value.equals(values[i])) idx = i;
        }

        if (idx == -1) System.out.println("Warning: illegal string value specified: " + value);
      }
    }
    ...
    void setStringValue(String v) {
      ...
      if (textField != null) textField.setText(v);
      if (comboBox != null) comboBox.setSelectedIndex(idx);
  ...
  protected void notifyListeners(String name) {
    for (ParameterListener pl : listeners) pl.parameterChanged(name);
  }
  ...
  public int gi(String name) {
    ...
    if (p instanceof StringValue) return ((StringValue) p).idx;
