...
public class BufferedRandomAccessFile {
  ...
  boolean bufferDirty =
      false; // buffer needs to be written back to disk? (If true, reads MUST use buffer.)

  byte[] buffer = new byte[BUFFER_SIZE];
  long bufferOffset = -1; // what file offset does this buffer start at?
  int bufferLength =
      -1; // how many bytes of the buffer are valid? ( < BUFFER_SIZE near end of file)
  ...
  /**
   * Invariant: the current file position = bufferOffset + bufferPosition. This position is always
   * stored inside the buffer, or this position is the byte after the current buffer (in which case
   * the next read will re-fill the buffer. *
   */
  ...
  void bufferSeek(long seekOffset) throws IOException {
    ...
    // System.out.printf("%08x %08x %08x %08x\n", seekOffset, bufferOffset, bufferPosition,
    // bufferLength);
  }

  public final int read() throws IOException {
    if (bufferOffset + bufferPosition >= fileLength) throw new EOFException("EOF");
  ...
  public byte peek() throws IOException {
    if (bufferPosition < bufferLength) return buffer[bufferPosition];
  ...
  */
  ...
  public void write(byte src[], int offset, int writelen) throws IOException {
    for (int i = offset; i < offset + writelen; i++) write(src[i]);
  }
  ...
  public static void main(String args[]) {
    try {
      ...
      String l;
      while ((l = in.readLine()) != null) System.out.printf("^%s$\n", l);
