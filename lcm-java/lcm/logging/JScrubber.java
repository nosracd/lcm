...
public class JScrubber extends JComponent {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (bookmarks.size() == 0) {
        ...
        for (Bookmark b : bookmarks) {
          if (b.position < position && (b0 == null || b0.position < b.position)) b0 = b;
          if (b.position > position && (b1 == null || b1.position > b.position)) b1 = b;
        }
      }
  ...
  double getPosition2(int x) {
    double pos = ((double) x - MARGIN) / (getWidth() - 2 * MARGIN);
    if (pos < 0) return zoom0;
    if (pos > 1) return zoom1;
  ...
  double getPosition(int x, int y) {
    ...
    if (getRow(x, y) == 0) position = getPosition(x);
    else position = getPosition2(x);

    return position;
  }

  /** Get position for a mouse click on a particular row Set row = 1 for the zoomed in slider. */
  ...
  void updateGeometry() {
    if (inhibitGeometryChanges) return;
  ...
  void drawBookmark(Graphics g, Bookmark b, int x, int cy) {
    ...
    if (b.type == BOOKMARK_LREPEAT) {
      ...
      g.fillRect(
          x + BOOKMARK_WIDTH / 2, cy - BOOKMARK_HEIGHT / 2, REPEAT_DOT_SIZE, REPEAT_DOT_SIZE);
      g.fillRect(
          x + BOOKMARK_WIDTH / 2,
          cy + BOOKMARK_HEIGHT / 2 - REPEAT_DOT_SIZE,
          REPEAT_DOT_SIZE,
          REPEAT_DOT_SIZE);
    }
    ...
    if (b.type == BOOKMARK_RREPEAT) {
      g.setColor(new Color(0, 50, 0));
      g.fillRect(
          x - BOOKMARK_WIDTH / 2 - REPEAT_DOT_SIZE,
          cy - BOOKMARK_HEIGHT / 2,
          REPEAT_DOT_SIZE,
          REPEAT_DOT_SIZE);
      g.fillRect(
          x - BOOKMARK_WIDTH / 2 - REPEAT_DOT_SIZE,
  ...
  public void paint(Graphics g) {
    ...
    for (Bookmark b : bookmarks) {
      ...
      if (b.position >= zoom0 && b.position <= zoom1) drawBookmark(g, b, getX2(b.position), cy2);
    }
  ...
  void userSet(double newpos) {
    ...
    for (JScrubberListener l : listeners) l.scrubberMovedByUser(this, position);
  ...
  public synchronized void set(double pos) {
    ...
    for (Bookmark b : bookmarks) {
      if (b.type == BOOKMARK_RREPEAT && b.position > oldpos && b.position <= pos) {
        ...
        for (Bookmark bl : bookmarks) {
          if (bl.position < b.position
              && bl.type == BOOKMARK_LREPEAT
              && (lrepeat == null || bl.position > lrepeat.position)) lrepeat = bl;
        }
  ...
  class MyMouseAdapter extends MouseInputAdapter {
    ...
    public void mouseClicked(MouseEvent e) {
      ...
      if (e.getButton() == 1) {
        ...
        if (nearest == null) userSet(position);
        else userSet(nearest.position);
      }
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      if (mouseDownRow == 1) inhibitGeometryChanges = true;
      else inhibitGeometryChanges = false;
