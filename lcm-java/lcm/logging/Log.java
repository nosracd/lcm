...
public class Log {
   ...
   * @param mode Specifies the access mode, must be one of "r", "rw", "rws", or "rwd". See {@link
   *     java.io.RandomAccessFile#RandomAccessFile RandomAccessFile} for more detail.
   */
  ...
  public synchronized Event readNext() throws IOException {
    ...
    while (true) {
      ...
      if (channellen <= 0 || datalen <= 0 || channellen >= 256 || datalen >= 2147483647) {
        System.out.printf(
            "Bad log event eventnumber = 0x%08x utime = 0x%08x channellen = 0x%08x"
                + " datalen=0x%08x\n",
  ...
  /**
   * Writes an event to the log file. The user is responsible for filling in the eventNumber field,
   * which should be sequentially increasing integers starting with 0.
   */
  ...
  /**
   * A convenience method for write. It internally manages the eventNumber field, and so calls to
   * this method should not be mixed with calls to the other write methods. *
   */
