...
public class LogPlayer extends JComponent {
  ...
  /**
   * We have events coming from all over the place: the UI, UDP events, callbacks from the
   * scrubbers. To keep things sanely thread-safe, all of these things simply queue events which are
   * processed in-order. doStop, doPlay, doStep, do(Anything) can only be called from the queue
   * thread.
   */
  ...
  class SeekEvent implements QueuedEvent {
    ...
    public void execute(LogPlayer lp) {
      ...
      if (player_was_running) doStop();

      doSeek(pos);

      if (player_was_running) doPlay();
    }
  }
  ...
  public LogPlayer(String lcmurl) throws IOException {
    ...
    fasterButton.setRolloverIcon(
        new ImageIcon(makeArrowImage(Color.magenta, getBackground(), false)));
    ...
    slowerButton.setRolloverIcon(
        new ImageIcon(makeArrowImage(Color.magenta, getBackground(), true)));
    ...
    p.add(speedLabel);
    p.add(fasterButton);
    //                         x  y    w          h  fillx   filly   anchor     fill        insets,
    //                 ix,   iy
    ...
    add(
        ...
        new GridBagConstraints(
            0, row, REMAINDER, 1, 1.0, 0.0, CENTER, HORIZONTAL, new Insets(0, 5, 0, 5), 0, 0));
    row++;
    ...
    add(
        ...
        new GridBagConstraints(
            0, row, REMAINDER, 1, 1.0, 1.0, CENTER, BOTH, new Insets(0, 0, 0, 0), 0, 0));
    row++;

    /// spacers

    add(
        Box.createHorizontalStrut(90),
    ...
    toggleAllButton.addActionListener(
        new ActionListener() {
    ...
    add(
        toggleAllButton,
        new GridBagConstraints(0, row, 2, 1, 0.0, 0.0, CENTER, NONE, insets, 0, 0));

    add(
        stepPanel,
        new GridBagConstraints(
            2, row, REMAINDER, 1, 1.0, 0.0, CENTER, HORIZONTAL, new Insets(0, 5, 0, 5), 0, 0));
    ...
    if (null == lcmurl) lcm = new LCM();
    else lcm = new LCM(lcmurl);

    logName.addMouseListener(
        new MouseAdapter() {
          public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2) openDialog();
          }
        });
    timeLabel.addMouseListener(
        new MouseAdapter() {
          public void mouseClicked(MouseEvent e) {
    ...
    public void scrubberExportRegion(JScrubber js, double p0, double p1) {
      ...
      try {
        ...
        while (inlog.getPositionFraction() < p1) {
          ...
          if (f != null && f.enabled) outlog.write(e);
        }
        inlog.close();
        outlog.close();
  ...
  void savePreferences() throws IOException {
    ...
    for (JScrubber.Bookmark b : bookmarks) {
      ...
      if (b.type == JScrubber.BOOKMARK_RREPEAT) type = "RREPEAT";
  ...
  Filter addChannelFilter(String channel, boolean enabledByDefault) {
    ...
    if (filteredPattern == null) f.enabled = enabledByDefault;
    else f.enabled = !(invertFilteredPattern ^ filteredPattern.matcher(channel).matches());
  ...
  void loadPreferences(String path) throws IOException {
    ...
    while ((line = ins.readLine()) != null) {
      ...
      if (toks[0].equals("BOOKMARK") && toks.length == 3) {
        ...
        if (toks[1].equals("RREPEAT")) type = JScrubber.BOOKMARK_RREPEAT;
        if (toks[1].equals("LREPEAT")) type = JScrubber.BOOKMARK_LREPEAT;
      ...
      if (toks[0].equals("CHANNEL") && toks.length == 4) {
        ...
        if (filteredPattern
            != null) // disable if either the saved value, or the filter say it should be disabled
        f.enabled =
              f.enabled
                  && !(invertFilteredPattern ^ filteredPattern.matcher(f.inchannel).matches());
      }
      if (toks[0].equals("ZOOMFRAC")) js.setZoomFraction(Double.parseDouble(toks[1]));
    }
  ...
  void populateChannelFilters() {
    try {
      ...
      while (true) {
        ...
        if (logStartUTime < 0) logStartUTime = e.utime;
  ...
  void setLog(String path, boolean startPlaying) throws IOException {
    if (currentLogPath != null) savePreferences();
  ...
  class PlayerThread extends Thread {
    ...
    public void run() {
      ...
      try {
        while (!stopflag) {
          // read position fraction before getting to next event to be able to get back to it if we
          // pause
  ...
  class FilterTableModel extends AbstractTableModel {
    ...
    public void setValueAt(Object v, int row, int column) {
      ...
      if (column == 1) f.outchannel = (String) v;
      if (column == 2) f.enabled = (Boolean) v;
    }
  }
  ...
  public static void main(String args[]) {
    ...
    if (System.getProperty("java.vendor").indexOf("Free Software Foundation") >= 0) {
      System.err.println(
          "WARNING: Detected gcj. The LCM log player is not known to work well with gcj.");
    ...
    for (optind = 0; optind < args.length; optind++) {
      ...
      if (c.equals("-h") || c.equals("--help")) {
        ...
        if (c.startsWith("--lcm-url=")) {
          ...
          if (parts.length == 2 && !parts[1].isEmpty()) optarg = parts[1];
        } else if (hasParam) {
          optind++;
          optarg = args[optind];
        }
    ...
    try {
      ...
      if (channelFilterRegex != null) p.setChannelFilter(channelFilterRegex);
      if (invertChannelFilter) p.invertChannelFilter();
