...
/**
 * Implementation of an error bar painter that allows configuration of the way the connection, start
 * point and end point of an error bar is painted by the use of {@link
 * info.monitorenter.gui.chart.IPointPainter}.
 *
 * <p>Property change events are fired as described in method <code>
 * {@link info.monitorenter.gui.chart.IErrorBarPainter#addPropertyChangeListener(String, PropertyChangeListener) }
 * </code> . Note that adding property change listeners to the nested access facades of type <code>
 * {@link info.monitorenter.gui.chart.IErrorBarPainter.ISegment}</code> accessible via <code>
 * getXXXSegment()</code> methods will fire the corresponding events for listeners of this instance
 * (as they delegate the calls) while they fire events for properties defined in <code>
 * {@link info.monitorenter.gui.chart.IErrorBarPainter.ISegment}</code> too. If you register for
 * events of this instance and for the retrieved segments you will receive two <code>
 * {@link PropertyChangeListener#propertyChange(java.beans.PropertyChangeEvent)}</code> for the same
 * value changed.
 *
 * <p>
 *
...
public class ErrorBarPainter implements IErrorBarPainter {
  ...
  /**
   * Base class for <code>ISegment</code> implementations that covers common features such as
  ...
  private abstract class ASegment implements ISegment {
     ...
     * </code>.
     *
     * <p>Note that adding property change listeners to the nested access facades of type <code>
     * {@link IErrorBarPainter.ISegment}</code> accessible via <code>getXXXSegment()</code> methods
     * will fire the corresponding events for listeners of this instance (as they delegate the
     * calls) while they fire events for properties defined in <code>
     * {@link IErrorBarPainter.ISegment}</code> too. If you register for events of this instance and
     * for the retrieved segments you will receive two <code>
     * {@link PropertyChangeListener#propertyChange(java.beans.PropertyChangeEvent)}</code> for the
     * same value changed.
     *
     * <p>
     *
     * @see
     *     info.monitorenter.gui.chart.IErrorBarPainter.ISegment#addPropertyChangeListener(java.lang.String,
  ...
  /**
   * The facade instance for accessing the connection segment of this configurable error bar
   * painter.
   *
   * <p>
   */
  private final ISegment m_segmentConnection =
      new ASegment() {
        ...
        /**
         * @see
         *     info.monitorenter.gui.chart.IErrorBarPainter.ISegment#getPropertySegmentPointPainter()
         */
        ...
        /**
         * @see
         *     info.monitorenter.gui.chart.IErrorBarPainter.ISegment#setPointPainter(IPointPainterConfigurableUI)
         */
   ...
   * <p>
   */
  private final ISegment m_segmentEnd =
      new ASegment() {
        /** Generated <code>serialVersionUID</code>. */
   ...
   * <p>
   */
  private final ISegment m_segmentStart =
      new ASegment() {
   ...
   *
   * <p>It then has to be configured with the remaining methods as desired.
   *
   * <p>
   *
  ...
  /**
   * Default color for all segments in case their <code>{@link IPointPainterConfigurableUI}</code>
   * has not been configured with a color.
   */
  ...
  /**
   * Internal routine that returns the default color in case the argument or it's color is null
   * (factored out code).
   *
   * <p>
   *
   * @param painter the painter to check for color.
   * @return the color of the given painter or <code> {@link #DEFAULT_SEGMENT_COLOR}</code> in case
   *     the color or the painter was null.
   */
  ...
  public int hashCode() {
    ...
    result =
        prime * result
            + ((this.m_connectionPainter == null) ? 0 : this.m_connectionPainter.hashCode());
    result =
        prime * result + ((this.m_endPointPainter == null) ? 0 : this.m_endPointPainter.hashCode());
    result =
        prime * result
    ...
    result =
        prime * result
            + ((this.m_segmentConnection == null) ? 0 : this.m_segmentConnection.hashCode());
    ...
    result =
        prime * result
            + ((this.m_startPointPainter == null) ? 0 : this.m_startPointPainter.hashCode());
    return result;
  }

  /**
   * @see info.monitorenter.gui.chart.IErrorBarPainter#paintErrorBar(int, int, ITracePoint2D,
   *     Graphics, IErrorBarPixel)
   */
  ...
  /**
   * Internally renders the given part of the error bar with support for color and invisibility
   * (null painter) management.
   *
   * <p>Factored out code to keep calling method {@link #paintErrorBar(int, int, ITracePoint2D,
   * Graphics, IErrorBarPixel)} smaller.
   *
   * <p>
   *
   * @param absoluteX the x coordinate in px of the error bar part.
   * @param absoluteY the y coordinate in px of the error bar part.
   * @param nextX the next x coordinate in px of the error bar part (only relevant for connection).
   * @param nextY the next y coordinate in px of the error bar part (only relevant for connection).
   * @param pointPainter the painter to use.
   * @param g2d needed for painting.
   * @param original the original trace point this error bar is painted for.
   */
  private final void paintErrorBarPart(
      final int absoluteX,
      final int absoluteY,
      final int nextX,
      final int nextY,
      final ITracePoint2D original,
      final IPointPainter<?> pointPainter,
      final Graphics g2d) {
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.IErrorBarPainter#removePropertyChangeListener(java.beans.PropertyChangeListener)
   */
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.IErrorBarPainter#removePropertyChangeListener(java.lang.String,
   *     java.beans.PropertyChangeListener)
   */
  ...
  public final void setConnectionColor(final Color connectionColor) {
    ...
    if (this.m_connectionPainter != null) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          IErrorBarPainter.PROPERTY_CONNECTION_COLOR, old, connectionColor);
    }
  }

  /**
   * @see
   *     info.monitorenter.gui.chart.IErrorBarPainter#setConnectionPainter(info.monitorenter.gui.chart.IPointPainterConfigurableUI)
   */
  public final void setConnectionPainter(final IPointPainterConfigurableUI<?> connectionPainter) {
    ...
    this.m_propertyChangeSupport.firePropertyChange(
        IErrorBarPainter.PROPERTY_CONNECTION, old, connectionPainter);
  }
  ...
  public final void setEndPointColor(final Color endPointColor) {
    if (this.m_endPointPainter != null) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          IErrorBarPainter.PROPERTY_ENDPOINT_COLOR, old, endPointColor);
    }
  }

  /**
   * @see
   *     info.monitorenter.gui.chart.IErrorBarPainter#setEndPointPainter(info.monitorenter.gui.chart.IPointPainterConfigurableUI)
   */
  public final void setEndPointPainter(final IPointPainterConfigurableUI<?> endPointPainter) {
    ...
    this.m_propertyChangeSupport.firePropertyChange(
        IErrorBarPainter.PROPERTY_ENDPOINT, old, endPointPainter);
  }
  ...
  public final void setStartPointColor(final Color startPointColor) {
    if (this.m_startPointPainter != null) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          IErrorBarPainter.PROPERTY_STARTPOINT_COLOR, old, startPointColor);
    }
  }

  /**
   * @see
   *     info.monitorenter.gui.chart.IErrorBarPainter#setStartPointPainter(IPointPainterConfigurableUI)
   */
  public final void setStartPointPainter(final IPointPainterConfigurableUI<?> startPointPainter) {
    ...
    this.m_propertyChangeSupport.firePropertyChange(
        IErrorBarPainter.PROPERTY_STARTPOINT, old, this.m_startPointPainter);
  }
}
