...
/**
 * Two charts aligned vertically (via {@link Chart2D#setSynchronizedXStartChart(Chart2D)}) that
 * display geometric forms backed by mathematics I do not understand (any more) ;-) .
 *
 * <p>
 *
...
public class VerticalStackedChartsWithParametricSpirals {
  public VerticalStackedChartsWithParametricSpirals(JPanel panel) {
    ...
    for (int i = 0; i < 10000; ++i) {
      double t = i / 10000.0;
      trace2.addPoint(
          Math.cos(t * 2 * Math.PI) + 0.2 * Math.cos(t * 20 * Math.PI),
    ...
    trace3.setStroke(
        new BasicStroke(
            1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 10, new float[] {10, 10}, 0));
  }
