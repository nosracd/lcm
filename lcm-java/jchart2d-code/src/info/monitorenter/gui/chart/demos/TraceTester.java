...
/**
 * A test class that steps through all <code>ITrace2D</code> implementations and adds random or
 * "half- random" <code>TracePoint2D</code> -instances.
 *
 * <p>You may see, that not all <code>ITrace2D</code> implementations work as proposed
 * (Trace2DLimitedReplacing). This will be fixed.
 *
 * <p>
 *
...
public final class TraceTester {
   ...
   *
   * <p>Unlike random points half random points are not totally random samples within the given
   * bounds but repeat the same x value 10 times.
   *
   * <p>
   *
  ...
  static class HalfRandomPoints extends TraceTester.RandomPoints {
    ...
    /**
     * Defines how often the same x value is returned from subsequent calls to {@link #nextPoint()}.
     */
    private double m_samexcount = 0;

    /**
     * Creates a half random point generator that will create random points within
     * ([minx..minx+maxx], [miny..miny+maxy]).
     *
     * <p>
     *
     * @param minx the lower x bound for points to generate.
     * @param maxx the x range for points to generate.
     * @param miny the lower y bound for points to generate.
     * @param maxy the y range for points to generate.
     */
  ...
  private static class RandomPoints {
    ...
    /**
     * Creates a random point generator that will create random points within ([minx..minx+maxx],
     * [miny..miny+maxy]).
     *
     * <p>
     *
    ...
    public ITracePoint2D nextPoint() {
      return new TracePoint2D(
          this.m_rand.nextDouble() * this.m_xrange + this.m_xmin,
          this.m_rand.nextDouble() * this.m_yrange + this.m_ymin);
    }
  }
