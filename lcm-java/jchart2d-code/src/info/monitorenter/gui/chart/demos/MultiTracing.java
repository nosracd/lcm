 ...
 * startup.
 *
 * <p>This test may blow your CPU. I am currently working on an AMD Athlon 1200, 512 MB RAM so I did
...
public final class MultiTracing extends JFrame {
  ...
  static final class AddPaintRemoveThread extends Thread {
     ...
     * @param trace the trace to add points to.
     * @param data the y values of the points to add.
     * @param sleep the length of the sleep break between painting points in ms.
     */
    public AddPaintRemoveThread(
        final Chart2D chart, final ITrace2D trace, final double[] data, final long sleep) {
      ...
      try {
        ...
        while (true) {
          ...
          if (Chart2D.DEBUG_THREADING) {
            System.out.println(
                this.getName() + "(" + Thread.currentThread().getName() + ") adding trace.");
          }
          this.m_innnerChart.addTrace(this.m_trace);
          for (int i = 0; i < this.m_data.length; i++) {
            if (Chart2D.DEBUG_THREADING) {
              System.out.println(
                  "Thread " + this.getName() + " adding point to " + this.m_trace.getName());
            }
          ...
          if (Chart2D.DEBUG_THREADING) {
            System.out.println(
                this.getName() + "(" + Thread.currentThread().getName() + ") removing trace.");
          }
   ...
   * <p>
   *
   * @param forceXRange the range that at least has to be kept visible.
   */
   ...
   * <p>
   *
   * @param forceYRange the range that at least has to be kept visible.
   */
