...
/**
 * An ILabelFormatter instance that uses a {@link java.text.DateFormat} to format the labels.
 *
 * <p><b>Caution: <br>
 * It only makes sense to use this class if the data of the corresponding axis may be interpreted as
 * long number of milliseconds since the standard base time known as "the epoch", namely January 1,
 * 1970, 00:00:00 GMT. </b>
 *
 * <p><b>Caution: <br>
 * This implementation is not completely conform with the constraint: <code>
 * instance.parse(instance.format(value)) == value
 * </code> </b> This only works for subsequent call: one call to format contains the next value to
 * return from parse to be the same as the format. That value is cached as date / time formatting
 * produces truncation of the internal value (e.g. if no year is displayed). <br>
 * Use: <br>
 *
 * <pre>
 *
...
public class LabelFormatterDate extends ALabelFormatter implements IAxisLabelFormatter {
  ...
  /**
   * Default constructor that uses the local datetime ( <code>
   * {@link DateFormat#getDateTimeInstance(int, int)}</code>) with <code>{@link DateFormat#SHORT}
   * </code> style.
   *
   * <p>
   */
  ...
  public boolean equals(final Object obj) {
    ...
    if (Double.doubleToLongBits(this.m_lastFormatted)
        != Double.doubleToLongBits(other.m_lastFormatted)) {
      return false;
    }
    return true;
  }
