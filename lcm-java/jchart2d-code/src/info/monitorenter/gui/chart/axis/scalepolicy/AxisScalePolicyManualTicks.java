 ...
 *
 * <ul>
 *   <li>No label will overwrite the following label.
 *   <li>No two labels will have the same value.
 *   <li>Every tick will exactly show the value without rounding errors.
 *   <li>Always the closest next possible tick is chosen regardless whether it is a major tick or a
 *       minor tick (subject to change in favor of major ticks)
 * </ul>
 *
 * <p>While this strategy is quite comfortable and prevents visual oddities there are some
 * consequences to it:
 *
 * <ul>
 *   <li>Major ticks are not guaranteed to be shown. This is because a label of a minor tick label
 *       may need so much space that the following major tick has to be skipped (subject to change)
 *   <li>Detailed control is not easy. E.g. if you want to enforce more ticks to show up you could:
 *       <ul>
 *         <li>Set an {@link LabelFormatterNumber} via {@link
 *             IAxis#setFormatter(IAxisLabelFormatter)} that formats little to no digits. But this
 *             could have both effects: More labels as the labels take less space or less labels as
 *             the value range is so little that an increased formatted value is possible only
 *             little times within that range.
 *         <li>Choose major and minor ticks via {@link IAxis#setMinorTickSpacing(double)} and {@link
 *             IAxis#setMajorTickSpacing(double)}
 *       </ul>
 *   <li>Performance is not the best. This is because the space for a label has to be computed and
 *       pixels have to be transformed from and to the value domain.
 * </ul>
 *
 * <p>
 *
...
public class AxisScalePolicyManualTicks implements IAxisScalePolicy {
   ...
   * @param majorTickSpacing if value % majorTickSpacing is zero then the resulting <code>
   *          {@link LabeledValue}</code> will be judged as a major tick.
   * @param axis needed to re-parse the value into the label string by using the formatter of it.
   * @return the value rounded to minor or major ticks.
   */
  protected LabeledValue createLabeledValue(
      final double value, final double majorTickSpacing, final IAxis<?> axis) {
