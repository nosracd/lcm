 ...
 * <code> double scaledX</code> and <code>double scaledY</code> which allow the <code>Chart2D</code>
 * to cache the scaled values (between 0.0 and 1.0) without having to keep a copy of the aggregators
 * (<code>ITrace2D</code>) complete tracepoints.
 *
 * <p>This avoids the necessity to care for the correct order of a set of scaled tracepoints copied
 * for caching purposes. Especially in the case of new <code>TracePoint2D</code> instances added to
 * a <code>ITrace2D</code> instance managed by a <code>Chart2D</code> there remains no
 ...
 * tracepoint by value - comparisons of x and y: The <code>TracePoint2D</code> passed to the method
 * <code>traceChanged(Chart2DDataChangeEvent e)</code> coded in the argument is the original. <br>
 *
 * <p>Why caching of scaled values for the coordinates? <br>
 * This takes more RAM but else for every <code>repaint()</code> invocation of the <code>Chart2D
 * </code> would force all tracepoints of all traces to be rescaled again.
 *
 * <p>A TracePoint2D will inform it's listener of type <code>ITrace</code> on changes of the
 * internal values.
 *
 * <p>
 *
...
public class TracePoint2D extends Point2D.Double implements ITracePoint2D {
   ...
   *
   * <p>A trace point should be contained only in one trace!
   *
   * <p>
   */
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.ITracePoint2D#addAdditionalPointPainter(info.monitorenter.gui.chart.IPointPainter)
   */
  public final boolean addAdditionalPointPainter(final IPointPainter<?> additionalPointPainter) {
    Boolean result;
    result =
        this.doSynchronized(
            new ICodeBlock<Boolean>() {

              @SuppressWarnings("synthetic-access")
              public Boolean execute() {
                final boolean res =
                    TracePoint2D.this.m_additionalPointPainters.add(additionalPointPainter);
                // for interpolated points listener may be null:
                if (res && TracePoint2D.this.m_listener != null) {
                  TracePoint2D.this.m_listener.firePointChanged(
                      TracePoint2D.this, ITracePoint2D.STATE_CHANGED);
                }
  ...
  public Object clone() {
    ...
    result.m_additionalPointPainters =
        new LinkedHashSet<IPointPainter<?>>(this.m_additionalPointPainters);
    return result;
  }
  ...
  public boolean equals(final Object obj) {
    ...
    if (java.lang.Double.doubleToLongBits(this.m_x)
        != java.lang.Double.doubleToLongBits(other.m_x)) {
      return false;
    }
    if (java.lang.Double.doubleToLongBits(this.m_y)
        != java.lang.Double.doubleToLongBits(other.m_y)) {
      return false;
    }
    return true;
  }
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.ITracePoint2D#getManhattanDistance(info.monitorenter.gui.chart.ITracePoint2D)
   */
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.ITracePoint2D#setListener(info.monitorenter.gui.chart.ITrace2D)
   */
  ...
  /**
   * Internal helper that invokes the given internal runnable if this trace point is already
   * connected to a trace. This is needed to avoid that changes on a trace point that are made
   * within a paint iteration while the point already has been painted (but the iteration is still
   * ongoing and the modification of the point -> request of a repaint to the chart is accumulated)
   * are not reflected in the UI (are not drawn).
   *
   * <p>Synchronization is following the idiom also used in the painting <code>{@link Chart2D}
   * </code>.
   *
   * @param runSynchronized code to execute synchronized.
   */
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.ITracePoint2D#removeAdditionalPointPainter(info.monitorenter.gui.chart.IPointPainter)
   */
  ...
  public Set<IPointPainter<?>> removeAllAdditionalPointPainters() {
    return this.doSynchronized(
        new ICodeBlock<Set<IPointPainter<?>>>() {
          @SuppressWarnings("synthetic-access")
