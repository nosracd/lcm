...
/**
 * Interface for an error bar in the pixel domain (vs. value domain) for a single {@link
 * info.monitorenter.gui.chart.TracePoint2D}.
 *
 * <p>Errors returned from the getters represent only the error part and not the absolute value.
 * Errors are always absolute values (vs. relative to the original value to add the error to.
 *
 * <p>
 *
...
public interface IErrorBarPixel extends Serializable {
  ...
  /**
   * Returns the negative error (positive value) in X dimension as a pixel value or {@link
   * #ERROR_PIXEL_NONE}.
   *
   * <p>
   *
  ...
  /**
   * Returns the negative error (positive value) in Y dimension as a pixel value or {@link
   * #ERROR_PIXEL_NONE}.
   *
   * <p>
   *
