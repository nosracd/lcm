...
/**
 * Additional to the <code>Trace2DLtdReplacing</code> all tracepoints will be sorted by their x-
 * value.
 *
 * <p>Performance is slower compared to the class named above. Internally a <code>TreeSet </code> is
 * used (instead of <code>RingBufferArrayFast</code>) to keep the comparable <code>TracePoint2D
 * </code>- instances sorted. Internally all tracepoints are <code>TracePoint2D</code> -instances.
 *
 * <p>
 *
...
public class Trace2DLtdSorted extends Trace2DSorted {
   ...
   *
   * @param maxsize the maximum amount of points to show.
   */
  ...
  /**
   * In case point has an x- value already contained, the old trace point with that value will be
   * replaced by the new one. Else the new trace point will be added at an index in order to keep
   * the ascending order of trace points with a higher x- value are contained.
   *
   * <p>If points takes additional space (it's x- value is not already contained) and maxsize is
   * reached, the first element (with lowest x- value) will be removed.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param amount the maximum amount of points that will be shown.
   */
