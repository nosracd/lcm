...
/**
 * A default superclass for IRangePolicy implementations that adds support for setting and getting
 * ranges.
 *
 * <p>Should be used by any implementation that really works on the data of ranges (not unbounded
 * ranges). Subclasses should access the internal member range or use {@link #getRange()}.
 *
 * <p>
 *
...
public abstract class ARangePolicy implements IRangePolicy {
  ...
  /**
   * The internal range that may be taken into account for returning bounds from {@link
   * IRangePolicy#getMax(double, double)} and {@link IRangePolicy#getMax(double, double)}.
   *
   * <p>
   */
   ...
   * <p>
   *
   * @param range the range that may be used to decide about the policy of displaying the range.
   */
   ...
   * <p>
   *
   * @return the internal range that may be taken into account for returning bounds from {@link
   *     IRangePolicy#getMax(double, double)} and {@link IRangePolicy#getMax(double, double)}.
   */
  ...
  /**
   * @see
   *     info.monitorenter.gui.chart.IRangePolicy#removePropertyChangeListener(java.beans.PropertyChangeListener,
   *     java.lang.String)
   */
  public void removePropertyChangeListener(
      final PropertyChangeListener listener, final String property) {
  ...
  public void setRange(final Range range) {
    ...
    if (minchanged && maxchanged) {
      ...
      this.firePropertyChange(
          IRangePolicy.PROPERTY_RANGE_MIN, Double.valueOf(oldMin), Double.valueOf(range.getMin()));
    } else if (maxchanged) {
      this.firePropertyChange(
          IRangePolicy.PROPERTY_RANGE_MAX, Double.valueOf(oldMax), Double.valueOf(range.getMax()));
    }
  }
}
