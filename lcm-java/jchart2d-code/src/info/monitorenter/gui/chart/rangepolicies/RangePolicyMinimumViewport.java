...
/**
 * A <code>{@link info.monitorenter.gui.chart.IRangePolicy}</code> implementation that guarantees a
 * minimum displayed range (viewport) but will stretch if values of the corresponding <code>
 * {@link info.monitorenter.gui.chart.Chart2D}</code> exceeds these constructor given bounds.
 *
 * <p>To sum up the policy of this implementation this <code>
 * {@link info.monitorenter.gui.chart.IRangePolicy}</code>
 *
 * <ol>
 *   <li>Guarantees to always display the constructor given range.
 *   <li>Guarantees to always display every value within the <code>
 *       {@link info.monitorenter.gui.chart.Chart2D}</code> (every <code>
 *       {@link info.monitorenter.gui.chart.TracePoint2D}</code> of the chart's <code>
 *       {@link info.monitorenter.gui.chart.ITrace2D}</code> instances).
 * </ol>
 *
 * <p>
 *
...
public class RangePolicyMinimumViewport extends ARangePolicy {
   ...
   *
   * @param range the range that always should be visible.
   */
   ...
   * @param chartMax returned if greater than the value of the internal range.
