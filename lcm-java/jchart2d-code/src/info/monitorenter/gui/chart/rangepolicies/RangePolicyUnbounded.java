...
public final class RangePolicyUnbounded extends ARangePolicy {
  ...
  /**
   * Creates a range policy backed by the given range which is not used for {@link #getMax(double,
   * double)} or {@link #getMin(double, double)} but stored for later changes of the range policy
   * from outside.
   *
   * <p>
   *
