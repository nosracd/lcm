...
/**
 * An interface used by <code>Chart2D</code>. ITrace2D contains the values to display, the color for
 * displaying and a suitable label. It may be seen as a trace of the <code>Chart2D</code> that
 * displays it. <br>
 * Implementations may be optimized for different use- cases: <br>
 * RingBuffers for fast changing data to keep the amount of trace points and consumption of memory
 ...
 *
 * <p>There are various constraints for Traces: <br>
 ...
 *
 * <p>{@link java.lang.Comparable} should be implemented by using the internal property zIndex (see
 ...
 *
 * The following <code>{@link java.beans.PropertyChangeEvent}</code> may be fired to <code>
 * {@link PropertyChangeListener}</code> instances that register themselves with <code>
...
public interface ITrace2D extends PropertyChangeListener, Comparable<ITrace2D>, Serializable {
   ...
   *
   * <p>TODO: change this to an interface and implement in abstract base class to hide constructor.
   *
   * <p>
   *
   * @author <a href="mailto:Achim.Westermann@gmx.de">Achim Westermann</a>
  ...
  /**
   * The property key defining the <code>color</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_COLOR = "ITrace2D.PROPERTY_COLOR";

  /**
   * The property key defining a change of <code>{@link IErrorBarPolicy}</code> instances contained.
   *
   * <p>This is fired from <code>{@link #addErrorBarPolicy(IErrorBarPolicy)}</code>, <code>
   * {@link #removeErrorBarPolicy(IErrorBarPolicy)}</code> and <code>
   * {@link #setErrorBarPolicy(IErrorBarPolicy)}</code>.
   *
   * <p>Use in combination with {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   *
   * <p>
   */
  ...
  /**
   * The property key defining a change of the configuration of a contained <code>
   * {@link IErrorBarPolicy}</code>.
   *
   * <p>This is fired whenever an <code>IErrorBarPolicy</code> notifies this instance of a
   * configuration change via an event for <code>{@link IErrorBarPolicy#PROPERTY_CONFIGURATION}
   * </code>.
   *
   * <p>Use in combination with {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   *
   * <p>
   */
  public static final String PROPERTY_ERRORBARPOLICY_CONFIGURATION =
      "ITrace2D.PROPERTY_ERRORBARPOLICY_CONFIGURATION";

  /**
   * The property key defining the <code>label</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   *
   * <p>This is no real property as the <code>{@link #getLabel()}</code> access two other fields.
   *
   * <p>
   */
  ...
  /**
   * The property key defining the <code>maxX</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MAX_X = "ITrace2D.PROPERTY_MAX_X";

  /**
   * The property key defining the <code>maxY</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MAX_Y = "ITrace2D.PROPERTY_MAX_Y";

  /**
   * The property key defining the <code>minX</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MIN_X = "ITrace2D.PROPERTY_MIN_X";

  /**
   * The property key defining the <code>minY</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_MIN_Y = "ITrace2D.PROPERTY_MIN_Y";

  /**
   * The property key defining the <code>name</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   * {@link ITracePainter}</code> instances. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   *
   * <p>Use in combination with {@link #addPropertyChangeListener(String, PropertyChangeListener)}.
   * Property change event with this property will be fired if method <code>
   * {@link TracePoint2D#setLocation(java.awt.geom.Point2D)}</code> or <code>
   * {@link TracePoint2D#setLocation(double, double)}</code> of any contained point is invoked.
   *
   * <p>
   */
  ...
  /**
   * The property key defining a change in the set of <code>
   * {@link IPointPainter}</code> instances. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_POINT_HIGHLIGHTERS_CHANGED =
      "ITrace2D.PROPERTY_POINT_HIGHLIGHTERS_CHANGED";

  /**
   * The property key defining the <code>stroke</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   * {@link TracePoint2D}</code> instances within this trace. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
  public static final String PROPERTY_TRACEPOINT = "ITrace2D.PROPERTY_TRACEPOINT";

  /**
   * The property key defining the <code>visible</code> property. Use in combination with {@link
   * #addPropertyChangeListener(String, PropertyChangeListener)}.
   */
   ...
   *
   * <p>the descriptive name for this trace.
   *
   * <p>
   *
   * @see #getZIndex()
   ...
   *
   * <p>The given trace will be informed in case an add operation on this trace succeeds via <code>
   * {@link #addPoint(ITracePoint2D)}</code>.
   *
   * @param trace the trace that will calculate it's points from the added points of this trace.
   */
   ...
   *
   * <p>It will be the last error bar policy to render (most forward on screen).
   *
   * <p>
   *
   * @param errorBarPolicy the error bar policy to add for rendering this trace's error bars.
   ...
   *
   * <p><b>Warning</b>:<br>
   * Do not call this method before this trace has been added to a chart or you will not succeed as
   * the chart is needed to get the proper <code>
   ...
   *
   * <p>Try to pass instances of <code>TracePoint2D</code> to this instance instead of invoking
   * <code>{@link #addPoint(double, double)}</code> to increase performance. Else the given point
   * has to be copied into such an instance from the other method and delegated to this method.
   *
   * <p>Implementations decide whether the point will be accepted or not. So they have to update the
   * internal properties <code>minX</code>, <code>maxX</code>,<code>maxY</code> and <code>minY
   * </code> and also care about firing property change events for those properties by method <code>
   ...
   *
   * <p>It will be the last point painter to paint highlighting if highlighting is active.
   *
   * <p>
   *
   * @param highlighter the highlighter to add for highlighting this trace.
   * @return true if the highlighter was added (class of instance not contained before).
   */
   ...
   * @param listener a listener that will only be informed if the property identified by the
   *     argument <code>propertyName</code> changes
   */
   ...
   *
   * <p>It will be the last painter to paint (most forward).
   *
   * <p>
   *
   * @param painter the painter to add for rendering this trace.
   ...
   * <p>
   *
   * @param painter the painter to check whether it is contained.
   ...
   *
   * <p>Bound checks are performed and property change events for the properties <code>
   * {@link ITrace2D#PROPERTY_MAX_X}</code>, <code>{@link ITrace2D#PROPERTY_MIN_X}</code>, <code>
   * {@link ITrace2D#PROPERTY_MAX_Y}</code> and <code>{@link ITrace2D#PROPERTY_MIN_Y}</code> are
   * fired if the add bounds have changed due to the modification of the point.
   *
   * <p>
   *
   * @param changed the point that has been changed which may be a newly added point (from <code>
   *     {@link #addPoint(ITracePoint2D)}</code>, a removed one or a modified one.
   * @param state one of {<code>{@link ITracePoint2D#STATE_ADDED},
   *          {@link ITracePoint2D#STATE_CHANGED},
   *          {@link ITracePoint2D#STATE_REMOVED}</code> to inform about the type of change.
   */
   ...
   *
   * <p>On the other hand only the corresponding <code>Chart2D </code> may detect the same color
   ...
   * color. Then the <code>Chart2D</code> will chose a color not owned by another <code>ITrace2D
   * </code> instance managed and assign it to the null- returning instance.
   *
   * <p>The <code>Chart2D </code> will often call this method. So try to cache the value in
  ...
  /**
   * Returns the <code>Set&lt;{@link IErrorBarPolicy}&gt;</code> that will be used to render error
   * bars for this trace.
   *
   * <p>
   *
   * @return the <code>Set&lt;{@link IErrorBarPolicy}&gt;</code> that will be used to render error
   *     bars for this trace.
   */
   ...
   *
   * <p>If this returns false, no error bars will be painted for this trace.
   *
   * <p>
   *
  ...
  /**
   * Callback method for the <code>Chart2D</code> that returns a <code>String</code> describing the
   * label of the <code>ITrace2D</code> that will be displayed below the drawing area of the <code>
   * Chart2D</code>.
   *
   * <p>This method should be implemented and finalized ASAP in the inheritance tree and rely on the
   ...
   *
   * <p>Implementations should be synchronized for multithreaded use. No exception is thrown. In
   * case of empty data (no tracepoints) 0 should be returned. (watch division with zero).
   *
   * <p>The <code>Chart2D </code> will often call this method. So try to cache the value in
   * implementation and only check on modifications of <code>TracePoint</code> instances or on
   * <code>add</code>- invocations for changes.
  ...
  /**
   * Returns the nearest point to the given normalized value coordinates of this trace in Euclid
   * distance.
   *
   * <p>Please note that the arguments must be normalized value coordinates like provided by a
   * <code>{@link TracePoint2D#getScaledX()}</code> or the division of a pixel coordinate by the
   * total pixel range of the chart.
   *
   ...
   * <p>
   *
   * @param x the x value as a normalized value between 0 and 1.0.
   * @param y the x value as a normalized value between 0 and 1.0.
   * @return the nearest point to the given normalized value coordinates of this trace in Euclid
   *     distance.
   */
  public DistancePoint getNearestPointEuclid(double x, double y);

  /**
   * Returns the nearest point to the given normalized value coordinates of this trace in Manhattan
   * distance.
   *
   * <p>Please note that the arguments must be normalized value coordinates like provided by a
   * <code>{@link TracePoint2D#getScaledX()}</code> or the division of a pixel coordinate by the
   ...
   * @param x the x value as a normalized value between 0 and 1.0.
   * @param y the x value as a normalized value between 0 and 1.0.
   * @return the nearest point to the given normalized value coordinates of this trace in Manhattan
   *     distance.
   */
  ...
  /**
   * Returns the <code>Set&lt;{@link IPointPainter}&gt;</code> that may be used to highlight points
   * of this trace.
   *
   * <p>This is used by the point highlighting feature: <code>
   * {@link Chart2D#enablePointHighlighting(boolean)}</code>
   *
   * @return the <code>Set&lt;{@link IPointPainter}&gt;</code> that may be used to highlight points.
   */
   ...
   * <p>
   *
   * @param property one of the constants with the <code>PROPERTY_</code> prefix defined in this
   *     class or subclasses.
   ...
   * trace.
   *
   * <p>The original set should be returned by contract to allow adding painters "silently" (vs.
   * using <code>{@link ITrace2D#addTracePainter(ITracePainter)}</code>).
   *
   * <p>
   *
   ...
   *
   * <p>A higher value will bring it more "to the front".
   *
   * <p>
   *
  ...
  /**
   * Returns false if internal <code>{@link TracePoint2D}</code> instances are contained or true if
   * not.
   *
   * <p>
   *
   * @return <tt>false</tt> if internal <code>{@link TracePoint2D}</code> instances are contained or
   *     <tt>true</tt> if not.
   */
   ...
   *
   * <p>Implementations should be synchronized. This method is meant to allow modifications of the
   * intenal <code>TracePoint2D</code> instances, so the original points should be returned.
   *
   * <p>There is no guarantee that changes made to the contained tracepoints will be reflected in
   * the display immediately. The order the iterator returns the <code>TracePoint2D</code> instances
   ...
   *
   * <p>Returns the <code>Set&lt;{@link IPointPainter}&gt;</code> that was used to highlight points.
   *
   * <p>This is used by the point highlighting feature: <code>
   * {@link Chart2D#enablePointHighlighting(boolean)}</code>
   *
   * @return the <code>Set&lt;{@link IPointPainter}&gt;</code> that was be used to highlight points.
   */
  public Set<IPointPainter<?>> removeAllPointHighlighters();

  /**
   * Removes all internal <code>TracePoint2D</code>.{@link #isEmpty()} will return true afterwards.
   *
   * <p>
   */
  ...
  /**
   * Remove a trace that "should" compute values from the points added via <code>
   * {@link #addPoint(ITracePoint2D)}</code>.
   *
   * <p>
   *
   * @param trace the trace that will calculate it's points from the added points of this trace.
   * @return true if the given trace was removed (recognized by the means of <code>
   *     {@link Object#equals(Object)}</code>).
   */
   ...
   * <p>
   *
   * @param errorBarPolicy the error bar policy to remove.
   ...
   * @return true if a point highlighter of the class of the given argument was removed.
   */
  public boolean removePointHighlighter(final IPointPainter<?> highlighter);

  /**
   * Unregisters a property change listener that has been registered for listening on all
   * properties.
   *
   * <p>
   *
   ...
   * @param property one of the constants with the <code>PROPERTY_</code> prefix defined in this
   *     class or subclasses.
   * @param listener the listener for this property change.
   */
   ...
   * <p>
   *
   * @param painter the trace painter to remove.
   ...
   * <p>
   *
   * @param color the <tt>Color</tt> to set.
   */
   ...
   * <p>
   *
   * @param errorBarPolicy the new sole painter to use.
  ...
  /**
   * Assingns a specific name to the <code>ITrace2D</code> which will be displayed by the <code>
   * Chart2D</code>.
   *
   * <p>
   *
   * @param name the name for this trace.
   */
  public void setName(String name);

  /**
   * Assigns a specific String representing the physical unit to the <code>
   *  ITrace2D</code> (e.g. Volt, Ohm, lux, ...) which will be displayed by the <code>
   * {@link Chart2D}</code>
   *
   * <p>
   *
   * @param xunit the physical unit for the x axis.
   * @param yunit the physical unit for the y axis.
   */
   ...
   * <p>
   *
   * @param highlighter the new sole highlighter to use.
   * @return the <code>Set&lt;{@link IPointPainter}&gt;</code> that was used before or null if
   *     nothing changed.
   */
   ...
   * elsewhere (needed for synchronization).
   *
   * <p>Not the best design to put this to an interface, but Char2D should handle this interface
   * only.
   *
   * <p>
   *
  ...
  /**
   * Allows to specify the rendering of the ITrace2D. This Stroke will be assigned to the {@link
   * java.awt.Graphics2D} by the rendering {@link Chart2D} when painting this instance.
   *
   * <p>
   *
   * @param stroke the stroke to use for painting this trace.
   */
   ...
   * <p>
   *
   * @param painter the new sole painter to use.
   ...
   * <p>
   *
   * @param visible true if this trace should be painted, false else.
   */
   ...
   *
   * <p>The higher the given value is the more this trace will be brought to front.
   *
   * <p>The value must not be lower than {@link #Z_INDEX_MIN}(0) and higher than {@link
   * #ZINDEX_MAX}(100).
   *
   * <p>This might not be tested for increased performance but ignoring these bounds may result in
   * wrong ordering of display.
   *
   * <p>
   *
   * @param zIndex the z index of this trace - the lower the value the more in front the trace will
   ...
   *
   * <ul>
   *   <li>this trace contains {@link IErrorBarPolicy} instances.
   ...
   *
   * <ul>
   *   <li>this trace contains at leaste one {@link IErrorBarPolicy} instance that {@link
