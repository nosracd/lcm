 ...
 * <p>
 *
 * @param <T> needed for generics <code>{@link #compareTo(Object)}</code>.
...
public interface IPointPainterConfigurableUI<T extends IPointPainter<T>> extends IPointPainter<T> {
   ...
   *
   * <p>This value will be computed from the color used. If that color is not configured (null) a
   * value of 0.0 is returned even if the color from the <code>{@link Graphics}</code> used for
   * painting has a different setting!
   *
   * <p>
   *
  ...
  /**
   * Sets the stroke to paint with or <code>null</code> if no special color is desired.
   *
   * <p>In the latter case the stroke of the <code>{@link Graphics}</code> provided for paint
   * operations will be used.
   *
   * <p>
   *
   * @param stroke the stroke to paint with.
   * @return the previous color being used or <code>null</code> if none was used before.
   */
   ...
   * any effect.
   *
   * <p>Caution: When using a value greater 0 may cost a multiple cpu load!
   *
   * <p>
   *
   * @param transparency0to255 a transparency value between 0 and 255.
   ...
   *
   * <p>This value will be fold into fill color. If fill color has not been configured before it
