...
public class AnnotationBubble extends JPanel {
   ...
   * <p>
   *
   * @param chartPanel the chart panel this instance is added to as an annotation.
   * @param annotationContent the content area (information container) of the visible annotation
   *     information.
   ...
   */
  public AnnotationBubble(
      final ChartPanel chartPanel,
      final AAnnotationContentComponent annotationContent,
   ...
   * <p>
   *
   * @param outlineDragMode the outlineDragMode to set
   */
   ...
   *
   * <p>Caution: When using a value greater 0 dragging the annotation may cost a multiple cpu load
   * and dragging speed! In this case consider setting the paintOnDragging property to false;
   *
   * @param zeroToOneAlpha a transparency value between 0.0 and 1.0.
   */
    ...
    synchronized (this) {
      ...
      if (zeroToOneAlpha != 0) {
        ...
        if (bgColor.getAlpha() != alphaInt) {
          this.setBackground(
              new Color(bgColor.getRed(), bgColor.getGreen(), bgColor.getBlue(), alphaInt));
        }
      } else {
