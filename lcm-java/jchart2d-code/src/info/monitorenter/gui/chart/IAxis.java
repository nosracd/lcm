...
public interface IAxis<T extends IAxisScalePolicy> extends Serializable {
  ...
  public final class AxisTitle implements Cloneable, Serializable {
    ...
    /**
     * Creates an instance with the given title backed by a <code>{@link AxisTitlePainterDefault}
     * </code>.
     *
     * <p>
     *
     ...
     *
     * <p>The following <code>{@link java.beans.PropertyChangeEvent}</code> types should be fired to
     * listeners:<br>
     *
     * <table border="0">
     * <tr>
    ...
    /**
     * Returns an array of all the listeners that were added to the this instance with {@link
     * #addPropertyChangeListener(String, PropertyChangeListener)}.
     *
     * <p>
     *
     * @return an array of all the listeners that were added to the this instance with {@link
     *     #addPropertyChangeListener(String, PropertyChangeListener)} .
     ...
     *
     * <p>Default is <code>{@link Color#BLACK}</code>.
     *
     * <p>
     *
     ...
     * <p>
     *
     * @param title the title or <code>null</code> if no title should be used.
    ...
    public void setTitleColor(final Color color) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          AxisTitle.PROPERTY_TITLECOLOR, old, this.m_titleColor);
    }
    ...
    public void setTitleFont(final Font font) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          AxisTitle.PROPERTY_TITLEFONT, old, this.m_titleFont);
    }
     ...
     * <p>
     *
     * @param titlePainter the titlePainter to set.
    ...
    public final IAxisTitlePainter setTitlePainter(final IAxisTitlePainter titlePainter) {
      ...
      this.m_propertyChangeSupport.firePropertyChange(
          AxisTitle.PROPERTY_TITLEPAINTER, old, this.m_titlePainter);
      return old;
    }
  }

  /**
   * The bean property <code>constant</code> identifying a change of the internal set of <code>
   * {@link ITrace2D}</code> instances.
   *
   * <p>Use this constant to register a {@link java.beans.PropertyChangeListener} with the <code>
   * IAxis</code>.
   *
   * <p>See {@link #addPropertyChangeListener(String, PropertyChangeListener)} for property change
   * events fired.
   */
  public static final String PROPERTY_ADD_REMOVE_TRACE = "IAxis.PROPERTY_ADD_REMOVE_TRACE";

  /**
   * The bean property <code>constant</code> identifying a change of the axis scale policy.
   *
   * <p>Use this constant to register a {@link java.beans.PropertyChangeListener} with the <code>
   * IAxis</code>.
   *
   ...
   * events fired.
   */
  public static final String PROPERTY_AXIS_SCALE_POLICY_CHANGED =
      "IAxis.PROPERTY_AXIS_SCALE_POLICY_CHANGED";

  /**
   * Constant for a <code>{@link java.beans.PropertyChangeEvent}</code> of the <code>
   * {@link IAxisTitlePainter}</code>.
   */
  ...
  /** Constant for a <code>{@link java.beans.PropertyChangeEvent}</code> of the paint scale flag. */
   ...
   * Add a listener for the given property.
   *
   * <p>The following {@link java.beans.PropertyChangeEvent} types should be fired to listeners:<br>
   *
   * <table border="0">
   * <tr>
   * <th><code>getPropertyName()</code></th>
   ...
   *
   * <p>Adding a trace that is already contained may be problematic, so an exception should be
   * raised in that case to warn you that your code is doing unnecessary to malicious operations.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @return {@link Chart2D#CHART_POSITION_LEFT}, {@link Chart2D#CHART_POSITION_RIGHT}, {@link
   *     Chart2D#CHART_POSITION_TOP}, {@link Chart2D#CHART_POSITION_BOTTOM} or -1 if this axis is
   ...
   *
   * <p>Prefer this method instead of <code>{@link #getAxisTitle()}</code> if you want to drop the
   * axis title as this method also "unlistens" this axis from it's title.
   *
   * <p>
   *
   ...
   *
   * <p><b>Note:</b></br/> For an y axis the hight only includes the overhang it needs on the upper
   ...
   * configured <code>{@link IRangePolicy}</code> (see <code>
   * {@link IAxis#setRangePolicy(IRangePolicy)}</code>). The returned value is either in x or y
   * dimension - depending on the dimension this axis is working in for the chart.
   *
   * <p>
   *
   * @return the maximum value of all <code>
   *         {@link info.monitorenter.gui.chart.TracePoint2D}</code> instances in all <code>
   *     {@link ITrace2D}</code> instances in this axis regardless of the configured <code>
   *     {@link IRangePolicy}</code> (see <code>{@link IAxis#setRangePolicy(IRangePolicy)}</code>).
   */
   ...
   * operation.
   *
   * <p>Note that this value is only valid throughout a <code>
   * {@link Chart2D#paint(java.awt.Graphics)}</code> invocation.
   *
  ...
  /**
   * Returns the range lasting from <code>{@link IAxis#getMin()}</code> to <code>
   * {@link IAxis#getMax()}</code>.
   *
   * <p>This method is used by the Chart2D to scale it's values during painting.
   *
   ...
   * data).
   *
   * <p>If the given absolute value is not in the display- range of the <code>Chart2D</code>,
   * negative values or values greater than 1.0 may result.
   *
   * <p>
   *
   * @param absolute a value in the real value range of the corresponding chart.
   ...
   * <p>
   *
   * @param axisScalePolicy the axis scale policy which controls the position and distance of the
   *     ticks to draw to use.
   ...
   *
   * <p><b>Caution!</b><br>
   * The original internal modifiable set is returned for performance reasons and by contract (to
   * allow removing traces) so do not mess with it to avoid ugly unpredictable side effects!
   *
   * <p>
   *
   ...
   * <p>This includes the axis line, it's ticks and labels and it's title.
   *
   * <p><b>Note:</b></br/> For an x axis the width only includes the overhang it needs on the right
  ...
  /**
   * Returns true if this axis is responsible for rendering the scale of the given trace (<code>
   * {@link IAxis#addTrace(ITrace2D)}</code> was called on this instance with the given trace).
   *
   * <p>
   *
   * @param trace the trace to check for containment.
   * @return true if this axis is responsible for rendering the scale of the given trace (<code>
   *     {@link IAxis#addTrace(ITrace2D)}</code> was called on this instance with the given trace).
   */
  ...
  /**
   * Returns true if the bounds in the given dimension of all {@link
   * info.monitorenter.gui.chart.TracePoint2D} instances of all internal {@link ITrace2D} instances
   * have changed since all points have been normalized to a value between 0 and 1 or true if this
   * axis has different range since the last call to <code>{@link IAxis#scale()}</code>.
   *
   * <p>
   *
   * @return true if the bounds in the given dimension of all {@link
   *     info.monitorenter.gui.chart.TracePoint2D} instances of all internal {@link ITrace2D}
   *     instances have changed since all points have been normalized to a value between 0 and 1 or
   *     true if this axis has different range since the last call to <code>
   ...
   *
   * <p>This should only be called from <code>{@link Chart2D}</code>, all other uses may cause
   ...
   *
   * <p><b>Intended for <code>{@link Chart2D}</code> only!!!</b>
   *
   * @param g2d needed for painting.
   ...
   * axis.
   *
   * <p>Implementations should fire a <code>{@link java.beans.PropertyChangeEvent}</code> for the
   * <code>{@link java.beans.PropertyChangeEvent#getPropertyName()}</code> <code>
   * {@link IAxis#PROPERTY_ADD_REMOVE_TRACE}</code> for every single trace removed. This is done
   * best by delegating this call to several calls to <code>{@link #removeTrace(ITrace2D)}</code>.
   *
   * <p>
   *
   ...
   *
   * <p>A <code>{@link java.beans.PropertyChangeEvent}</code> for the <code>
   * {@link java.beans.PropertyChangeEvent#getPropertyName()}</code> <code>
   * {@link IAxis#PROPERTY_ADD_REMOVE_TRACE}</code> has to be fired on the registered <code>
   * {@link PropertyChangeListener}</code> for the trace removed.
   *
   * <p>
   *
   * @param trace the trace to remove from this axis.
   ...
   *
   * <p>This method is not deadlock - safe and should be called by the <code>{@link Chart2D}</code>
   * only!
   *
   * <p>
   */
   ...
   * <p>
   *
   * @param axisTitle the axis title to use.
   */
   ...
   *
   * <p>The number that is passed-in represents the distance, measured in values, between each minor
   ...
   *
   * <p>A repaint operation for the chart is triggered.
   *
   * <p>
   *
   ...
   *
   * <p>This must only be called from the <code>{@link Chart2D}</code> itself!
   *
   * <p>
   *
   * @param pixel the left pixel coordinate of this axis in the graphic context of the current paint
   *     operation.
   */
   ...
   * <p>
   *
   * @param pixel the right pixel coordinate of this axis in the graphic context of the current
   *     paint operation.
   */
   ...
   * <p>
   *
   * @param pixel the bottom pixel coordinate of this axis in the graphic context of the current
   *     paint operation.
   */
   ...
   * <p>
   *
   * @param pixel the top pixel coordinate of this axis in the graphic context of the current paint
   *     operation.
   */
   ...
   * <p>
   *
   * @param painter the instance that will paint the title of this axis.
   ...
   * <p>
   *
   * @param visible true to paint axis, false to hide.
   */
  public void setVisible(boolean visible);

  /**
   * Transforms the given pixel value (which has to be a awt value like {@link
   * java.awt.event.MouseEvent#getY()} into the chart value.
   *
   * <p>Internal use only, the interface does not guarantee that the pixel corresponds to any valid
