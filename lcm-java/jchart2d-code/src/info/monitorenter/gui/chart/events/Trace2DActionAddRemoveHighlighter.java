...
/**
 * <code>Action</code> that adds or removes constructor-given {@link
 * info.monitorenter.gui.chart.IPointPainter} to the corresponding trace.
 *
 * <p>This action only works in combination with {@link AbstractButton} instances that send
 * themselves as the event object to the {@link java.awt.event.ActionEvent}( {@link
 * java.util.EventObject#getSource()} ) because <code>{@link AbstractButton#isSelected()}</code> is
 * needed for state check.
 *
 * <p>
 *
...
public final class Trace2DActionAddRemoveHighlighter extends ATrace2DAction {
   ...
   * @param hightlighter the highlighter to add / remove from the trace.
   */
  public Trace2DActionAddRemoveHighlighter(
      final ITrace2D trace, final String description, final IPointPainter<?> hightlighter) {
