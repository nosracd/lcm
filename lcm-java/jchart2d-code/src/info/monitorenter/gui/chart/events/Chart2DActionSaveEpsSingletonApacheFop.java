...
/**
 * Singleton <code>Action</code> that saves the current chart to an encapsulated postscript file at
 * the the location specified by showing a modal file chooser save dialog.
 *
...
public final class Chart2DActionSaveEpsSingletonApacheFop {
  ...
  /**
   * Implementation of the action that depends on jlibeps and must not be class loaded in case the
   * corresponding classes are missing in the classpath.
   */
  private static final class Chart2DActionSaveEps extends AChart2DAction {
     ...
     *     javax.swing.AbstractButton} subclasses that get this <code>Action</code> assigned (
    ...
    public void actionPerformed(final ActionEvent e) {
      ...
      if (ret == JFileChooser.APPROVE_OPTION) {
        File file =
            new File(this.m_filechooser.getSelectedFile().getAbsolutePath() + "." + extension);
        // get the encoding
        try {
          ...
          //          org.sourceforge.jlibeps.epsgraphics.EpsGraphics2D g = new
          // org.sourceforge.jlibeps.epsgraphics.EpsGraphics2D(
          //              "Title", outStream, 0, 0, this.m_chart.getWidth(),
          // this.m_chart.getHeight());
          // g.scale(0.24, 0.24);
  ...
  public static Action getInstance(final Chart2D chart, final String actionName) {
    Action result =
        Chart2DActionSaveEpsSingletonApacheFop.instances.get(
            Chart2DActionSaveEpsSingletonApacheFop.key(chart));
    if (result == null) {
      ...
      Chart2DActionSaveEpsSingletonApacheFop.instances.put(
          Chart2DActionSaveEpsSingletonApacheFop.key(chart), result);
    }
    return result;
  }
