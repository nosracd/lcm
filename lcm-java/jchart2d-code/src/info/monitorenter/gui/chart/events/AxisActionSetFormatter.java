 ...
 * info.monitorenter.gui.chart.axis.AAxis#setFormatter(info.monitorenter.gui.chart.IAxisLabelFormatter)}
 ...
 * <h2>Caution</h2>
 *
 * This implementation only works if assigned to a {@link AbstractButton}: It assumes that the
 * source instance given to {@link #actionPerformed(ActionEvent)} within the action event is of that
 * type as the state information (turn grid visible or turn grid invisible) is needed.
 *
 * <p>
 *
...
public class AxisActionSetFormatter extends AAxisAction {
   ...
   * identifies itself with the given action String and invokes {@link
   * info.monitorenter.gui.chart.axis.AAxis#setPaintGrid(boolean)} on the axis upon selection.
   *
   * <p>
   *
      ...
      if (this.m_formatter.equals(evt.getOldValue())) {
        if (!this.m_formatter.equals(evt.getNewValue())) {
          this.firePropertyChange(
              PropertyChangeCheckBoxMenuItem.PROPERTY_SELECTED, Boolean.TRUE, Boolean.FALSE);
        }
      } else {
        if (this.m_formatter.equals(evt.getNewValue())) {
          this.firePropertyChange(
              PropertyChangeCheckBoxMenuItem.PROPERTY_SELECTED, Boolean.FALSE, Boolean.TRUE);
        }
      }
    }
  }
