...
/**
 * Data collector that collects data in form of {@link info.monitorenter.gui.chart.ITracePoint2D}
 * instances from a property file ( {@link java.util.Properties}).
 *
 * <p>
 *
...
public class PropertyFileStaticDataCollector extends AStaticDataCollector {
  ...
  /**
   * The input stream in {@link java.util.Properties} format where {@link
   * info.monitorenter.gui.chart.TracePoint2D} are parsed from.
   *
   * <p>
   */
   ...
   * <p>
   *
   * @param trace the target trace to add data to.
   * @param propertyFileStream the stream of the file in the {@link java.util.Properties} format
   *     where {@link info.monitorenter.gui.chart.TracePoint2D} instances (key is x, value is y) is
   *     parsed from.
   */
  public PropertyFileStaticDataCollector(
      final ITrace2D trace, final InputStream propertyFileStream) {
    super(trace);
