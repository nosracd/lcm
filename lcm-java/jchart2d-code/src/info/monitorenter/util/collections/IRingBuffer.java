 ...
 * <p>
 *
 * @param <T> the type of instances to store in implementations of this ring buffer.
...
public interface IRingBuffer<T> extends java.io.Serializable, Iterable<T> {
   ...
   *
   * <p>If the buffer is full, an Exception will be thrown.
   *
   * <p>Note that <code>RingBufferException</code> does not need to be caught because it is an
   * inheritant of <code>java.lang.RuntimeException</code>. Especially for the <code>
   * Object add(Object element)</code>- method there may be an implementation that never throws
   * <code>BUFFER_FULL</code> but returns the oldest element in case the buffer is full.
   *
   * <p>
   *
   * @param element the element to add.
   * @return the instance that had to be removed in order to add the new one or null, if the
   *     capacity was not reached yet.
   * @throws IRingBuffer.RingBufferException if the buffer cannot accept any more elements.
   */
   ...
   *
   * <p>If the content is of no interest prefer using this method instead of {@link #removeAll()} as
   * it may be implemented in a much faster way ( <code>O(constant)</code> instead of <code>O(n)
   * </code>).
   *
   * <p>
   */
   ...
   * @throws IRingBuffer.RingBufferException if the buffer is empty.
   */
  ...
  /**
   * Returns true if no more space in the buffer is available. This method should be used to test
   * before calling {@link #add(Object)}.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @return an iterator starting from the first (youngest) to the last (oldest) element.
   */
   ...
   *
   * @return an iterator starting from the last (oldest) to the first (youngest) element.
   */
   ...
   *
   * <p>Implementations may vary on handling the problem that the new size is smaller than the
   * actual amount of elements in the buffer: <br>
   ...
   * <p>
   *
   * @param newSize the new buffer size to set.
   */
