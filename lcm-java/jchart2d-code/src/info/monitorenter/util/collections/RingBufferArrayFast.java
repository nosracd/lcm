 ...
 *
 * <p>This implementation differs from the <code>RingBufferArray</code> in one point: <br>
 * If <code>setBufferSize(int asize)</code> decreases the size of the buffer and it will get smaller
 * than the actual amount of elements stored, they will get lost. This avoids the need for an
 * internal List to store elements overhanging. Some tests may be left out that may speed up this
 * <code>IRingBuffer</code>. Adding 5000000 elements was about 25 % faster compared to the <code>
 * RingBufferArray</code> on an Athlon 1200, 256 MB RAM.
 *
 * <p>For allowing high performance single-threaded use this implementation and the implementations
 * of the retrievable <code>Iterator</code>- instances are not synchronized at all.
 *
 * <p>
 *
...
public class RingBufferArrayFast<T> implements Cloneable, IRingBuffer<T> {
  ...
  /**
   * Base for ring buffer iterators that has access to the ring buffer by being an non-static inner
   * class.
   *
   * <p>
   *
  ...
  protected abstract class ARingBufferIterator implements Iterator<T> {
    ...
    public T next() {
      ...
      if (result == null) {
        throw new NoSuchElementException(
            "RingBufferArrayFast.iteratorF2L returns null: pos:"
                + this.m_pos
                + " count: "
                + this.m_count);
      }
   ...
   *
   * <p>For performance reasons the size of the buffer -1!
   *
   * <p>
   */
   ...
   * <p>
   *
   * @param anObject the instance to add.
   * @return the oldest Object, if RingBuffer was filled with 'maxsize' elements before, or null.
   */
  @SuppressWarnings("unchecked")
  ...
  public boolean isFull() {
    ...
    if (RingBufferArrayFast.DEBUG) {
      System.out.println(
          "isFull: " + ret + " head: " + this.m_headpointer + " tail: " + this.m_tailpointer);
    }
    return ret;
  }
  ...
  /**
   * Returns an <code>Iterator</code> that will return the elements in exactly the inverse order the
   * subsequent call to <code>remove()</code> would do.
   *
   * <p>The youngest elements are returned first. <b>The <code>Iterator</code> returned is not
   * thread- safe! </b>
   *
   * <p>
   *
   * @return an <code>Iterator</code> that will return the elements in exactly the inverse order the
   *     subsequent call to <code>remove()</code> would do.
   */
  public java.util.Iterator<T> iteratorF2L() {
    return new ARingBufferIterator() {
      {
        this.m_pos =
            (RingBufferArrayFast.this.m_headpointer == 0)
                ? RingBufferArrayFast.this.size() - 1
                : RingBufferArrayFast.this.m_headpointer - 1;
      }
   ...
   * subsequent call to <code>remove()</code> would do.
   *
   * <p>The oldest elements are returned first. <b>The <code>Iterator</code> returned is not thread-
   * safe! </b>
   *
   * <p>
   *
   ...
   *
   * <p>A new size is assigned but the elements "overhanging" are returned by the <code>
   * Object remove()</code>- method first. This may take time until the buffer has its actual size
   * again. Don't pretend on calling this method for saving of memory very often as the whole buffer
   * has to be copied into a new array every time- and if newSize < getSize() additional the
   * overhanging elements references have to be moved to the internal <code>List pendingremove
   * </code>.
   *
   ...
   *
   * <p>Don't call this in your application too often: hard arraycopy - operation an malloc are
   * triggered.
   *
   * <p>
   *
