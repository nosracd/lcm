 ...
 *
 * <p>As soon as the maximum number of entries is reached, the next entry is added to the end and
 * the first entry is removed from it. In this case, all elements are stored in a Object[]. But
 * don't worry there will not be a single call to <code>System.arraycopy</code> caused by invocation
 * of the <code>add(Object element)</code>- method. Internal indexes into the array for the head and
 * tail allow to reuse the same memory again and again. <br>
 * No element is lost: If <code>setBufferSize(int asize)</code> decreases the size of the buffer and
 * it will get smaller than the actual amount of elements stored, they will get cached until
 * removed.
 *
...
public class RingBufferArray<T> extends RingBufferArrayFast<T> {
  ...
  /**
   * Base class for ring buffer iterators with pending removals (those who do not drop instances if
   * size is exceeded).
   *
   * <p>
   *
  ...
  abstract class ARingBufferIterator extends RingBufferArrayFast<T>.ARingBufferIterator {
    ...
    public T next() {
      ...
      if (this.m_pendpos >= 0) {
        ...
        if (ret == null) {
          System.out.println(
              "RingBufferArray.iteratorF2L returns null: head:"
                  + RingBufferArray.this.m_headpointer
                  + " tail: "
  ...
  /**
   * Elements that stores elements that have to be removed due to an invocation to {@link
   * #setBufferSize(int)} with a smaller argument than the amount of elements stored.
   */
  ...
  public int hashCode() {
    ...
    int result = 1;
    result =
        prime * result + ((this.m_pendingremove == null) ? 0 : this.m_pendingremove.hashCode());
    return result;
  }
  ...
  public java.util.Iterator<T> iteratorF2L() {
    return new ARingBufferIterator() {
      {
        this.m_pos =
            (RingBufferArray.this.m_headpointer == 0)
                ? RingBufferArray.this.m_size
   ...
   *
   * <p>Don't call this in your application too often: hard array copy - operation and memory
