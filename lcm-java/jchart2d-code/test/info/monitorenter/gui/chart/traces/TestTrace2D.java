...
public class TestTrace2D extends TestCase {
  ...
  public void testAddRemoveManyTrace2DLtd() {
    ...
    msg.append(']');
    Assert.assertEquals(
        "Wrong number of traces contained: " + msg.toString(), 5, chart.getTraces().size());
    for (int i = 0; i < 5; i++) {
    ...
    msg.append(']');
    Assert.assertEquals(
        "Wrong number of traces contained: " + msg.toString(), 0, chart.getTraces().size());
  ...
  /**
   * Adds and removes a trace point to a trace and asserts that zero listeners are in the trace
   * point afterwards.
   *
   * <p>
   */
  ...
  public void testPropertyChange() {
    Chart2D chart = new Chart2D();
    /**
     * Helper class to detect if <code>{@link PropertyChangeEvent}</code> are received as expected.
     *
     * <p>
     *
     * @author <a href="mailto:Achim.Westermann@gmx.de">Achim Westermann</a>
