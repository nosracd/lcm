...
public class TestChartPanel extends TestCase {
   ...
   *
   * <p>Warning: For now this is only a visual test, especially for seeing if the space for the
   * labels works correctly. This test will never fail if sth. is wrong. So watch it!
   *
   * <p>
   */
  ...
  public void testMemoryLeakAddRemoveTrace() {
    ...
    Map<String, Integer> listeners2CountBeforeAddTrace =
        TestChartPanel.getPropertyChangeListenerStatisticsForChartPerProperty(chart);

    Map<Class<?>, Integer> listenerTypes2CountBeforeAddTrace =
        getPropertyChangeListenerStatisticsForChartPerListenerInstance(chart);
    ...
    for (int i = 0; i < 20; i++) {
      ...
      try {
        if (i % 20 == 0) {
          System.out.print(
              "Running garbage collection and finalization (I know: never do this)...");
          System.gc();
    ...
    // reporting / analysis
    Map<String, Integer> listeners2CountAfterAddTrace =
        TestChartPanel.getPropertyChangeListenerStatisticsForChartPerProperty(chart);
    Map<Class<?>, Integer> listenerTypes2CountAfterAddTrace =
        getPropertyChangeListenerStatisticsForChartPerListenerInstance(chart);

    System.out.println();
    System.out.println("Before add trace:");
    for (Map.Entry<String, Integer> entry : listeners2CountBeforeAddTrace.entrySet()) {
      System.out.println(entry.getKey() + " : " + entry.getValue());
    }

    System.out.println();
    System.out.println("After add trace:");
    for (Map.Entry<String, Integer> entry : listeners2CountAfterAddTrace.entrySet()) {
      System.out.println(entry.getKey() + " : " + entry.getValue());
    }
    /*
    ...
    for (Class<?> listenerClass : listenerTypes2CountAfterAddTrace.keySet()) {
      ...
      if (amountAfter.intValue() > amountBefore.intValue()) {
        System.err.println(
            "Listener instances of type "
                + listenerClass.getName()
                + " on the chart have increased from "
                + amountBefore
                + " to "
                + amountAfter
                + ".");
        ...
        System.out.println(
            "Listener instances of type "
                + listenerClass.getName()
                + " on the chart remained constant before and after adding trace at "
                + amountBefore
                + ".");
      }
    }
     ...
     */
    Set<String> distinctListenedPropertiesBeforeAddRemoveTrace =
        listeners2CountBeforeAddTrace.keySet();
    Set<String> distinctListenedPropertiesAfterAddRemoveTrace =
        listeners2CountBeforeAddTrace.keySet();
    for (String propertyBefore : distinctListenedPropertiesBeforeAddRemoveTrace) {
      assertTrue(
          "Property "
              + propertyBefore
              + " was listened on (on the chart) before adding and removing a trace, but not"
              + " afterwards.",
          distinctListenedPropertiesAfterAddRemoveTrace.contains(propertyBefore));
    }
    for (String propertyAfter : distinctListenedPropertiesAfterAddRemoveTrace) {
      assertTrue(
          "Property "
              + propertyAfter
              + " is listened on (on the chart) after adding and removing a trace, but not before.",
          distinctListenedPropertiesBeforeAddRemoveTrace.contains(propertyAfter));
    }
    /*
  ...
  /**
   * Helper method that analyzes the <code>{@link PropertyChangeListener}</code> instances of a
   * given chart: it returns a map that contains the property to listen on (key) along with the
   * count of listeners of that property (value).
   *
   * <p>This may be used to find out, if the amount of listeners on a certain property increases
   * in-between invocation of a method that affects the chart.
   *
   * <p>
   *
   * @param chart the chart to analyze the listeners of.
   * @return a map that contains the property to listen on (key) along with the count of listeners
   *     of that property (value).
   */
  ...
  /**
   * Helper method that analyzes the <code>{@link PropertyChangeListener}</code> instances of a
   * given chart: it returns a map that contains listener <code>{@link Class}</code>(key) along with
   * the count of instances registered on the given chart (value).
   *
   * <p>This may be used to find out the implementation class that messes up, if the amount of
   * listeners of that type increases in-between invocation of a method that affects the chart.
   *
   * <p>
   *
   * @param chart the chart to analyze the listeners of.
   * @return a map that contains listener <code>{@link Class}</code>(key) along with the count of
   *     instances registered on the given chart (value).
   */
  private static Map<Class<?>, Integer>
      getPropertyChangeListenerStatisticsForChartPerListenerInstance(final Chart2D chart) {
    Class<?> clazz;
    Integer count;
    PropertyChangeListener[] propertyChangeListeners = chart.getPropertyChangeListeners();
